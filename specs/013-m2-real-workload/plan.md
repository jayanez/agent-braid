...
2. Validate the manifest with `scripts/validate_real_workload_manifest.py`.
   ...
   ambiguous inputs.
3. Obtain founder acceptance of ADR 0015 and the exact corpus before adding
   the test-command execution stage. Reuse the pinned T013 Docker base, then
   materialize candidate and serial private trees in separate containers.
